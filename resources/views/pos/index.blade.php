                    ...
                    <div class="col-md-6 mb-2 mb-md-0">
                        <div class="form-group mb-0">
                            <input class="form-control form-control-lg" type="text" name="keyword"
                                placeholder="{{ translate('Search by Product Name/Product Code/NHSSC') }}"
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="d-flex border-bottom pb-3">
                            ...
                            <button type="button" class="btn btn-icon btn-soft-dark text-white ml-3 mr-0"
                                onclick="getShippingAddress()" data-target="#new-customer" data-toggle="modal">
                        ...
                        <div class="" id="cart-details">
                                    ...
                                    <li class="list-group-item py-0 pl-2">
                                        <div class="row gutters-5 align-items-center">
                                            <div class="col-auto w-60px">
                                                <div
                                                    ...
                                                    <input type="text" name="qty-{{ $key }}" id="qty-{{ $key }}"
                                                        ...
                                                        min="{{ $stock->product->min_qty }}" max="{{ $stock->qty }}"
                                            ...
                                            <div class="col">
                                                ...
                                                <span class="span badge badge-inline fs-12 badge-soft-secondary">{{
                                                    $cartItem['variant'] }}</span>
                                            </div>
                                            <div class="col-auto">
                                                <div class="fs-12 opacity-60">{{ single_price($cartItem['price']) }} x
                            ...
                            <div>
                                ...
                                <div class="d-flex justify-content-between fw-600 fs-18 border-top pt-2">
                                    ...
                                    <span>{{ single_price($subtotal+$tax+Session::get('pos.shipping', 0) -
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between">
                        <div class="d-flex">
                                ...
                                <div class="dropdown-menu p-3 dropdown-menu-lg">
                                    <div class="input-group">
                                        <input type="number" min="0" placeholder="Amount" name="shipping"
                                            class="form-control" value="{{ Session::get('pos.shipping', 0) }}" required
                                ...
                                <div class="dropdown-menu p-3 dropdown-menu-lg">
                                    <div class="input-group">
                                        <input type="number" min="0" placeholder="Amount" name="discount"
                                            class="form-control from-sm" value="{{ Session::get('pos.discount', 0) }}" required
                            ...
                            <div class="dropdown dropup">
                                ...
                                <div class="dropdown-menu p-3 dropdown-menu-sm">
                                    <div class="input-group">
                                        <input type="number" min="0" placeholder="Amount" name="vat"
                                            class="form-control" id="vat" value="{{ Session::get('pos.tax', 0) }}" required onchange="setVat()">
                                        <div class="input-group-append">
                                            <span class="input-group-text">{{ translate('Flat') }}</span>
                                        </div>
                                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <div class=" row">
                            ...
                            <div class="col-sm-10">
                                <input type="number" min="0" placeholder="{{translate('Postal code')}}" id="postal_code"
                                    name="postal_code" class="form-control" required>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
        ...
        <div class="modal-content" id="variants">
            ...
            <div class="modal-footer">
                ...
                <button type="button" onclick="submitOrder('cash')"
                    class="btn btn-base-1 btn-success">{{translate('Confirm')}}</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <div class=" row">
                        <label class="col-sm-3 control-label" for="offline_payment_method">{{translate('Payment
                            Method')}}</label>
                        <div class="col-sm-9">
...
<script type="text/javascript">
    var products = null;
        ...
        function updateVatTax()
        {
            let vat = $('#vat').val();
            $.post('{{ route('pos.set_vat') }}', {_token:AIZ.data.csrf, vat:vat}, function(data){
                updateCart(data);
            })
        ...
        function filterProducts(){
            ...
            $.get('{{ route('pos.search_product') }}',{keyword:keyword,  brand:brand}, function(data){
                products = data;
                            ...
                            <div class="card-body p-2 p-xl-3">
                                ...
                                <div class="">
                                    ${data.data[i].price != data.data[i].base_price
                                        ...
                                        : `<span>£${data.data[i].unit_price}</span>`
                                    }

                                     <small>${data.data[i].product_code}</small>   
                                </div>
                            </div>
        ...
        function setShipping($is_custom_shipping_charges){
            var shipping = $('input[name=shipping]').val();
            $.post('{{ route('pos.setShipping') }}',{_token:AIZ.data.csrf, shipping:shipping, is_custom_shipping_charges:$is_custom_shipping_charges}, function(data){
                updateCart(data);
                updateVatTax()
            });
        }
        ...
        function getShippingAddress(){
            console.log("cust_id",$('select[name=customer_id]').val());
            $.post('{{ route('pos.getShippingAddress') }}',{_token:AIZ.data.csrf, customer_id:$('select[name=customer_id]').val()}, function(data){
        ...
        function add_new_address() {
            ...
            document.getElementById('set_customer_id').value = customer_id;
            
            var myModal = new bootstrap.Modal(document.getElementById('new-address-modal'));
            myModal.show();
        }
            ...
            function orderConfirmation() {
                ...
                let modal = new bootstrap.Modal(document.getElementById('order-confirm'));
                modal.show();

                // Send the AJAX request using Fetch API
                fetch('{{ route('pos.getOrderSummary') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': AIZ.data.csrf // Ensure CSRF token is sent
                    },
                    body: JSON.stringify({})
                })
                .then(response => response.text())
                .then(data => {
                    document.getElementById('order-confirmation').innerHTML = data;
                })
                .catch(error => {
        ...
        function submitOrder(payment_type){
            ...
            var purchase_number = $('input[name=purchase_number]').val();
            var order_id = $('input[name=order_id]').val();
            var pharmaceutical_checkbox = $('input[name=pharmaceutical_checkbox]').val();
            ...
            $.post('{{ route('pos.order_place') }}',{
                ...
                pharmaceutical_checkbox   : pharmaceutical_checkbox,
