<div class="aiz-pos-cart-list mb-4 mt-3 c-scrollbar-light">
    ...
    <ul class="list-group list-group-flush">
        ...
        $is_custom_shipping_charges = Session::get('is_custom_shipping_charges');
        // @dd($shipping, $is_custom_shipping_charges);
        if(!@$shipping || !$is_custom_shipping_charges) {
        $shipping_cost = $subtotal > 49.00 ? 0 : 7.99;
        Session::put('pos.shipping', $shipping_cost);
        }
        ...
        <li class="list-group-item py-0 pl-2">
            <div class="row gutters-5 align-items-center">
                <div class="col-auto w-60px">
                    <div class="row no-gutters align-items-center flex-column aiz-plus-minus">
                        ...
                        <input type="text" name="qty-{{ $key }}" id="qty-{{ $key }}"
                            ...
                            value="{{ $cartItem['quantity'] }}" min="{{ $stock->product->min_qty }}"
                            max="{{ $stock->qty }}" onchange="updateQuantity({{ $key }})">
                ...
                <div class="col-auto">
                    <div class="fs-12 opacity-60">{{ single_price($cartItem['price']) }} x {{ $cartItem['quantity'] }}
                    </div>
...
<div>
    ...
    <div class="d-flex justify-content-between fw-600 mb-2 opacity-70">
        ...
        {{-- <input type="hidden" name="tax_vat" id="tax_vat" value="{{ $tax }}"> --}}
    </div>
    <div class="d-flex justify-content-between fw-600 fs-18 border-top pt-2">
        <span>{{translate('Total Amount')}}</span>
        <span>{{ single_price($subtotal+Session::get('pos.tax')+Session::get('pos.shipping', 0) -
            Session::get('pos.discount', 0))
            }}</span>
    </div>
</div>