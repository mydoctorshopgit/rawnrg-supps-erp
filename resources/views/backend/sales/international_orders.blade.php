        ...
        <div class="card-body">
            <table class="table aiz-table mb-0">
                ...
                <tbody>
                    ...
                    $postcode = $shipping['post_code'] ?? $shipping['postal_code'] ?? null;
                    ...
                    if ($postcode && isset($postcodeCounts[$postcode]) && $postcodeCounts[$postcode] > 1) {
                    ...
                    // For guest orders (no user_id), pull details from shipping_address JSON
                    $isGuest       = empty($order->user_id);
                    $guestName     = trim(($shipping['name'] ?? ''));
                    $guestPostcode = $postcode ?? 'N/A';
                    @endphp
                    ...
                    <tr>
                        ...
                        <td>
                            @if($isGuest)
                                <span class="badge-secondary">Guest</span>
                            @elseif ($order->company_name != null && $order->company_name != '')
                                {{ $order->company_name }}
                            @else
                                N/A
                            @endif
                        ...
                        <td>
                            @if($isGuest)
                                {{ $guestName ?: 'Guest' }}<br>
                                <span class="badge-secondary">Guest</span>
                            @elseif ($order->customer_details != null)
                        ...
                        <!--    {{ $order->delivery_name }}-->
                        <!--</td>-->
                        <td><span style="background-color: {{ $bgColor }}; padding: 2px">{{ $guestPostcode }}</span></td>
                        ...
                        <td>
                            ...
                            @endphp
                            @if($isGuest)
                                @if(isset($payment_details_parsed->chargeId) && isset($payment_details_parsed->transactionId))
                                    <span>{{ translate('Complete') }}</span>
                                @else
                                    <span>{{ translate('UnComplete') }}</span>
                                @endif
                            @elseif ($order->User)
                                @if ($order->user->user_type == 'customer_credit')
                                    <span>{{ translate('Credit') }}</span>
                                @else
                                    @if($order->payment_details == 'cash' || $order->payment_details == null || !isset($payment_details_parsed->chargeId))
                                        <span>{{ translate('UnComplete') }}</span>
                                    @elseif(isset($payment_details_parsed->chargeId) && isset($payment_details_parsed->transactionId))
                                        <span>{{ translate('Complete') }}</span>
                                    @endif
                                @endif
                            @endif
                        ...
                        <td class="text-right">
                            ...
                            $order_detail_route = route('inhouse_orders.show', encrypt($order->id));
                             ...
                             if (Route::currentRouteName() == 'international_orders.index') {
                            $order_detail_route = route('international_orders.show', $order->id);
                            }
                            @endphp
