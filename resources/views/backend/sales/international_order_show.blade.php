    ...
    <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <small>
                                    ...
                                    @endphp
                                    {{translate('SKU')}}: {{ $orderDetail->sku }}
                                </small><br>
                                <small>Pack qty: {{ $stock->pack_qty ?? ''
                                    }}</small>
                                @elseif ($orderDetail->product != null && $orderDetail->product->auction_product == 1)
                                <strong>
                            ...
                            <td class="text-center">
                                <a class="edit-button btn btn-soft-primary btn-icon btn-circle btn-sm" href="#"
        ...
        <script>
            ...
            function attachQuantityChangeEventListener(input) {
                input.addEventListener('input', function () {
                    ...
                    const subTotalCell = input.closest('tr').querySelector('.sub-total');
                    subTotalCell.textContent = newSubTotal;

                    // Update hidden inputs for quantity and subtotal
                    const hiddenQtyInput = input.closest('tr').querySelector('.hidden-qty');
                    const hiddenSubTotalInput = input.closest('tr').querySelector('.hidden-subtotal');
                    hiddenQtyInput.value = newQuantity;
                    hiddenSubTotalInput.value = newSubTotal;
                });
            }
        ...
        <div class="clearfix float-right">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="discount-cell">
                            ...
                            <i class="la la-edit text-primary ms-2" style="cursor:pointer;"
                                onclick="openDiscountModal({{ $order->id }}, {{ $order->coupon_discount }})"></i>
                        </td>
                    </tr>


                    {{-- Include Bootstrap icons and modal --}}

                    <tr>
                        <td><strong class="text-muted">{{ translate('Delivery Charge') }} :</strong></td>
                        <td>
                            {{ single_price($order->orderDetails->sum('shipping_cost')) }}
                            <i class="la la-edit text-primary ms-2" style="cursor:pointer;"
                                onclick="openDeliveryModal({{ $order->id }}, {{$order->orderDetails->sum('shipping_cost') }})"></i>

                        </td>
                    </tr>
                    <tr>
                        <td><strong class="text-muted">{{ translate('VAT') }} :</strong></td>
                        <td>
                            {{ single_price($order->total_tax) }}
                            <i class="la la-edit text-primary ms-2" style="cursor:pointer;"
                                onclick="openVatModal({{ $order->id }}, {{ $order->total_tax }})"></i>
            ...
            <div class="no-print text-left">
                <button type="button" class="btn  btn-primary" id="send_to_new"><i class="las la-arrow"></i>Send To
                    New Order </button>
...
<div class="modal fade" id="editDeliveryModal" tabindex="-1" aria-labelledby="editDeliveryModalLabel"
    ...
    <div class="modal-dialog">
        <form method="POST" action="{{ route('orders.update_charge_popup') }}">
            @csrf

            <input type="hidden" name="order_id" id="delivery_order_id" value="{{ $order->id }}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <label class="form-label">New Delivery Charge</label>
                    <input type="number" id="delivery_value" name="value" step="0.01"
                        value="{{ $order->shipping_cost }}" class="form-control" required>
                </div>
...
<div class="modal fade" id="editVatModal" tabindex="-1" aria-labelledby="editVatModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('orders.update_charge_popup') }}">
            @csrf
            <input type="hidden" name="order_id" id="vat_order_id" value="{{ $order->id }}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <label class="form-label">New VAT Amount</label>
                    <input type="text" name="value" id="vat_value" value="{{ $order->total_tax }}" class="form-control"
                        required>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary">Update</button>
...
<div class="modal fade" id="editDiscountModal" tabindex="-1" aria-labelledby="editDiscountModalLabel"
    ...
    <div class="modal-dialog">
        <form method="POST" action="{{ route('orders.update_charge_popup') }}">
            @csrf
            <input type="hidden" name="order_id" id="discount_order_id" value="{{ $order->id }}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="number" name="value" id="discount_value" step="0.01"
                        value="{{ $order->coupon_discount }}" class="form-control" required>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary">Update</button>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
{{-- edit model --}}
<script>
    function openDeliveryModal(orderId, currentValue) {
        document.getElementById('delivery_order_id').value = orderId;
        document.getElementById('delivery_value').value = currentValue;

        let deliveryModal = new bootstrap.Modal(document.getElementById('editDeliveryModal'));
        deliveryModal.show();
    }

    function openVatModal(orderId, currentValue) {
        document.getElementById('vat_order_id').value = orderId;
        document.getElementById('vat_value').value = currentValue;

        let vatModal = new bootstrap.Modal(document.getElementById('editVatModal'));
        vatModal.show();
    }

    function openDiscountModal(orderId, currentValue) {
        document.getElementById('discount_order_id').value = orderId;
        document.getElementById('discount_value').value = currentValue;

        let discountModal = new bootstrap.Modal(document.getElementById('editDiscountModal'));
        discountModal.show();
    }
</script>
