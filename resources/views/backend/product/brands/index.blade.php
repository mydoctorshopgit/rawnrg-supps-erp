		                    ...
		                    <tr>
		                        ...
		                        <td>
		                            <input type="number"
		                                   ...
		                                   min="0"
		                                   data-brand-id="{{ $brand->id }}"
		                                   value="{{ $brand->order_level ?? 0 }}"
		                                   title="{{ translate('Lower number = higher priority') }}"
		                                   onblur="saveOrder(this)"
		                                   onkeydown="if(event.key==='Enter'){event.preventDefault();saveOrder(this);}">
		                        </td>
		                        <td class="text-right">
									@can('edit_brand')
					...
					<form action="{{ route('brands.store') }}" method="POST">
						...
						<div class="form-group mb-3">
							...
							<div class="input-group" data-toggle="aizuploader" data-type="image">
								...
								<input type="hidden" name="logo" class="selected-files" data-image-alt="logo_alt" data-image-alt-type="multiple" >
							</div>
...
<script type="text/javascript">
    ...
    function saveOrder(input) {
        var id    = input.getAttribute('data-brand-id');
        var order = parseInt(input.value, 10);

        if (isNaN(order) || order < 0) {
            Swal.fire({ icon: 'warning', title: 'Invalid', text: 'Order must be a number 0 or greater.', timer: 2000, showConfirmButton: false });
            return;
        }

        fetch("{{ route('brands.order.order_level') }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
            body: JSON.stringify({ id: id, order_level: order })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
