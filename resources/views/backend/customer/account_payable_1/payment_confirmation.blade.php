        ...
        <!--@endphp-->
                            <tr>
                               ...
                               <td>{{$order->user->name ?? "" . ' ' . $order->user->last_name ?? ""}}
                                ...
                                <td> 
                                    <a 
                                        href="#" class="btn btn-soft-primary btn-icon btn-circle btn-sm" 
                                        prim-id="{{$order->id}}" 
                                        cust-id="{{$order->order_id}}" 
                                        po_no="{{$order->order->purchase_order_number}}" 
                                        cname="{{$order->user->name  . ' ' . $order->user->last_name}}"  
                                        invoice_number="{{$order->order->invoice_number ?? 'N/A'}}" 
                                        due_date="{{$order->due_date ?? 'N/A'}}" 
                                        total="{{debit_price($order->order_id) ?? 'N/A'}}" 
                                ...
                                <td>  
                                    ...
                                    <a class="btn btn-soft-success btn-icon btn-circle btn-sm"  href="{{route('payment_ref', $remittancec->payment_ref)}}"  title="{{ translate('View') }}">
                                     ...
                                     <a 
                                        ...
                                        remaining_invoice="{{$remittancec->remaining_invoice ?? 'N/A'}}" 
            ...
            <div class="modal-body">
                <form action="{{ route('payment.confirm') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="invoice_numbers" name="invoice_numbers" class="form-control" value="" readonly required>
                    </div>
                    <div class="form-group">
                        <label for="paymentDate" class="form-label">Payment Date</label>
                        <input type="date" id="paymentDate" name="payment_date" class="form-control" value="" readonly required>
                    </div>
                    <div class="form-group">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('payment.confirm') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="paymentDate" class="form-label">Payment Date</label>
                        <input type="date" id="paymentDate2" name="payment_date" class="form-control" value="" readonly required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group" id="remainingAmountSection">
                        <span id="remainingAmountMessage" style="display:block; text-align:center; font-weight:bold; color: #d9534f;"></span>
                    </div>
