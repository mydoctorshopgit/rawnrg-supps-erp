...
if (!function_exists('getDeliveryAddress')) {
    function getDeliveryAddress($customer_id, $post_code)
    {
        return DeliveryAddress::where('customer_detail_id', '=', $customer_id)->where('postcode', '=', $post_code)->first();
    }
}
...
// get all active countries
if (!function_exists('get_signle_countries')) {
    function get_signle_countries($id)
    {
        $country_query = Country::where('id', '=', $id)->first();
        return $country_query->name;
    }
}
...
if (!function_exists('cart_product_price')) {
    function cart_product_price($cart_product, $product, $formatted = true, $tax = true)
    {
        if ($product->auction_product == 0) {
            ...
            $price = 0;
            $product_stock = $product->stocks->where('sku', $cart_product['sku'])->first()
                ?? $product->stocks->where('pip_code', $cart_product['sku'])->first();
            if ($product_stock) {
...
if (!function_exists('cart_product_tax')) {
    ...
    {
        // Look up stock by SKU first, then by pip_code, then fall back to first stock
        $product_stock = $product->stocks->where('sku', $cart_product['sku'] ?? '')->first()
            ?? $product->stocks->where('pip_code', $cart_product['sku'] ?? '')->first()
            ?? $product->stocks->first();

        // If still no stock found, use 0 as price base for tax calculation
        ...
        return $formatted ? format_price(convert_price($tax)) : $tax;
    }
}
...
if (!function_exists('pos_products_price')) {
    function pos_products_price($id, $customer_id = 0)
    {
        $product_stock = ProductStock::findOrFail($id);
        // dd($customer_id);
        //  $cusotmer_price = CustomerProductsAssign::where('products_id','=',$product_stock->product_id)->where('customer_detail_id','=',$customer_id)->first();
...
if (!function_exists('due_price')) {
    function due_price($id)
    {
        $debit_price = Accounts::where('debit', "!=", '0')->where('order_id', '=', $id)->sum("debit");
        $credit_price = Accounts::where('credit', "!=", '0')->where('order_id', '=', $id)->sum("credit");
        $amount_withheld = Accounts::where('order_id', '=', $id)->sum("amount_withheld");

        $price = ($debit_price + $amount_withheld) - $credit_price;

        return format_price(convert_price($price));
    }
}

if (!function_exists('debit_price')) {
    function debit_price($id)
    {
        $debit_price = Accounts::where('debit', "!=", '0')->where('order_id', '=', $id)->sum("debit");


        return format_price(convert_price($debit_price));
    }
}

if (!function_exists('credit_price')) {
    function credit_price($id)
    {
        $credit_price = Accounts::where('credit', "!=", '0')->where('order_id', '=', $id)->sum("credit");


        return format_price(convert_price($credit_price));
    }
}

if (!function_exists('due_price_customer')) {
    function due_price_customer($id)
    {
        $debit_price = Accounts::where('debit', "!=", '0')->where('customer_detail_id', '=', $id)->where('status', '=', '1')->sum("debit");
        $credit_price = Accounts::where('credit', "!=", '0')->where('customer_detail_id', '=', $id)->where('status', '=', '1')->sum("credit");
        $amount_withheld = Accounts::where('customer_detail_id', '=', $id)->where('status', '=', '1')->sum("amount_withheld");
...
if (!function_exists('due_price_without_currency')) {
    function due_price_without_currency($id)
    {
        $debit_price = Accounts::where('debit', "!=", '0')->where('order_id', '=', $id)->sum("debit");
        $credit_price = Accounts::where('credit', "!=", '0')->where('order_id', '=', $id)->sum("credit");

        $price = $debit_price - $credit_price;
...
if (!function_exists('uploaded_asset')) {
    ...
    {
        if (($asset = Upload::find($id)) != null) {
            return $asset->file_name == null ? $asset->external_link : my_asset($asset->file_name, null, $size);
        }
        return static_asset('assets/img/placeholder.jpg');
    }
}
...
if (!function_exists('my_asset')) {
     ...
     * @return string
     */
    function my_asset($path, $secure = null, $size = null)
    {
        // return $path;
        if ($size) {
            $imagePath = explode('all/', $path)[1];
            $resizedPath = str_replace($imagePath, $size . '-' . $imagePath, $path);
...
// if (!function_exists('stock_price')) {
//     function stock_price($stock_price)
//     {
//         $data = [];

//         foreach ($stock_price as $stock_prices) {

//             $data[] = [
//                 'product_id' => $stock_prices->product_id,
//                 'variant'    => $stock_prices->variant,
//                 'sku'        => $stock_prices->sku,
//                 'pip_code'   => $stock_prices->pip_code,
//                 'order'      => $stock_prices->order,
//                 'price'      => $stock_prices->price,
//                 'qty'        => $stock_prices->qty,
//             ];
//         }
...
// }
// if (!function_exists('stock_price')) {
//     function toFixedDown($number, $decimals = 2) {
...
//         foreach ($stock_price as $stock_prices) {
//             $product = ProductTax::where('product_id', $stock_prices->product_id)->first();

//             // If product is not found, assume 0% VAT
//             $vat = $product ? $product->tax : 0;
...
//                 'sku'        => $stock_prices->sku,
//                 'pip_code'   => $stock_prices->pip_code,
//                 'thumbnail_img'   => uploaded_asset($stock_prices->thumbnail_img),,
//                 'photos'   => collect(json_decode($stock_prices->photos, true) ?? explode(',', $stock_prices->photos))
//                 ->filter() 
//                 ->map(function ($photo) {
//                 return uploaded_asset(trim($photo));
//                  })->toArray(),
//                 'description'   => $stock_prices->description,
//                 'short_description'   => $stock_prices->short_description,
//                 'order'      => $stock_prices->order,
//               'price'       => number_format($stock_prices->price, 2, '.', ''),
// 'total_price' => toFixedDown($stock_prices->price + ($stock_prices->price * $vat / 100), 2),
//                 'qty'        => $stock_prices->qty,
//             ];
//         }
...
if (!function_exists('stock_price')) {
    ...
    function stock_price($stock_price, $slug = null)
    {
        $data = [];

        foreach ($stock_price as $stock) {
            $productTax = ProductTax::where('product_id', $stock->product_id)->first();
            $vat = $productTax ? $productTax->tax : 0;

            $productCode = ($stock->sku != 0 && !empty($stock->sku))
                ? $stock->sku
                : $stock->pip_code;
            ...
            // Build full slug (with SKU)
            $slugParts = [rtrim($stock->product->slug, '-')];

            if (!empty($stock->variant) && $stock->variant != 0) {
                $slugParts[] = trim(strtolower(preg_replace('/[^A-Za-z0-9]+/', '-', trim(str_replace(['"', "'", "\u{201C}", "\u{201D}", "\u{2018}", "\u{2019}"], '', $stock->variant)))), '-');
            }
            if (!empty($stock->color)) {
                $slugParts[] = trim(strtolower(preg_replace('/[^A-Za-z0-9]+/', '-', trim(str_replace(['"', "'", "\u{201C}", "\u{201D}", "\u{2018}", "\u{2019}"], '', $stock->color)))), '-');
            }
            if (!empty($stock->flavour)) {
                $slugParts[] = trim(strtolower(preg_replace('/[^A-Za-z0-9]+/', '-', trim(str_replace(['"', "'", "\u{201C}", "\u{201D}", "\u{2018}", "\u{2019}"], '', $stock->flavour)))), '-');
            }
            if (!empty($productCode)) {
                $slugParts[] = $productCode;
            }

            $fullSlug = implode('-', array_filter($slugParts, fn($p) => $p !== ''));

            // Also generate clean size slug for matching
            $cleanSizeSlug = rtrim($stock->product->slug, '-');
            if (!empty($stock->variant) && $stock->variant != 0) {
                $cleanSizeSlug .= '-' . trim(strtolower(preg_replace('/[^A-Za-z0-9]+/', '-', trim(str_replace(['"', "'", "\u{201C}", "\u{201D}", "\u{2018}", "\u{2019}"], '', $stock->variant)))), '-');
            }
            ...
            $data[] = [
                ...
                'slug'              => $fullSlug,
                'clean_size_slug'   => $cleanSizeSlug,        // ← NEW: Helpful for matching
                'size'              => $stock->variant,
                ...
                'thumbnail_img'     => uploaded_asset($stock->thumbnail_img),
                'images'            => collect(json_decode($stock->photos, true) ?? explode(',', $stock->photos))
                    ->filter(fn($p) => !empty(trim((string) $p)))
                    ->map(fn($photo) => uploaded_asset(trim($photo)))
                    ->filter(fn($url) => $url !== static_asset('assets/img/placeholder.jpg'))
                    ->values()
                    ->toArray(),
                ...
                'net_price'         => number_format($stock->price,2,'.',''),
                'has_discount'      => $has_discount,
                'discount'          => "-" . discount_in_percentage($product) . "%",
                'price'             => number_format($discount_price,2,'.',''),
                'vat'               => number_format($productVat,2,'.',''),
                // 'total_price'       => single_price($stock->price + round($stock->price * $vat / 100, 2)),
                'total_price'       => number_format($finalPrice,2,'.',''),
                'qty'               => $stock->qty,
                'current_slug'      => false,   // We will set this in Resource
            ];
        }
...
if (!function_exists('stock_price_by_size')) {

    function stock_price_by_size($stock_price, $slug = null)
    {
        $data = [];
        ...
        foreach ($stock_price as $stock_prices) {
            ...
            // Product code
            $productCode = ($stock_prices->sku != 0 && !empty($stock_prices->sku))
                ? $stock_prices->sku
                : $stock_prices->pip_code;

            // Build slug using flavor > size+color > base
            $slugParts = [$stock_prices->product->slug];

            if (!empty($stock_prices->flavour)) {
                // Flavor only
                $slugParts[] = strtolower(
                    preg_replace('/[^A-Za-z0-9]+/', '-', trim($stock_prices->flavour))
                );
            } elseif (!empty($stock_prices->variant) && $stock_prices->variant != 0) {
                // Size
                $slugParts[] = strtolower(
                    preg_replace('/[^A-Za-z0-9]+/', '-', trim($stock_prices->variant))
                );

                // Color (only if exists)
                if (!empty($stock_prices->color)) {
                    $slugParts[] = strtolower(
                        preg_replace('/[^A-Za-z0-9]+/', '-', trim($stock_prices->color))
                    );
                }
            }
            ...
            $data[] = [
                ...
                'product_id'        => $stock_prices->product_id,
                'slug'              => $generatedSlug,
                'size'              => $stock_prices->variant,
                'flavour'           => $stock_prices->flavour,
                'color'             => $stock_prices->color,
                'sku'               => $stock_prices->sku,
                'pack_qty'          => $stock_prices->pack_qty,
                'pip_code'          => $stock_prices->pip_code,
                'thumbnail_img'     => uploaded_asset($stock_prices->thumbnail_img),
                'photos'            => collect(json_decode($stock_prices->photos, true) ?? explode(',', $stock_prices->photos))
                    ->filter()
                ...
                'total_price'       => toFixedDown($stock_prices->price + ($stock_prices->price * $vat / 100), 2),
                'qty'               => $stock_prices->qty,
                'current_slug'      => isset($slug) && $slug === $generatedSlug,
            ];
        }
        ...
        $grouped = collect($data)->groupBy(function ($item) {
            ...
            $hasNoSize = ($size === '' || $size === '0' || $size === null);

            if ($hasNoSize) {

                if (!empty($item['flavour'])) {
                    return 'flavour_' . $item['flavour'];
                }
            ...
            if (str_starts_with($key, 'size_')) {
                return [
                    'type' => 'size',
                    'name' => str_replace('size_', '', $key),
                    'items' => $items->values(),
                ];
            }

            if (str_starts_with($key, 'flavour_')) {
                return [
                    'type' => 'flavour',
                    'name' => str_replace('flavour_', '', $key),
                    'items' => $items->values(),
                ];
            }
            return [
