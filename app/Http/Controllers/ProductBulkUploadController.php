...
use App\Models\ImportCustomerProducts;
...
class ProductBulkUploadController extends Controller
{
     ...
     public function customer_products()
    {
        if (Auth::user()->user_type == 'seller') {
            ...
            $data["all_customers"] = CustomerDetail::all();
            $data["customer_products"] = CustomerProductsAssign::paginate(10);
            // dd($data["all_customers"]);
            return view('backend.product.bulk_upload.customer_products')->with($data);
        }
    }
    ...
    public function bulk_upload(Request $request)
    {
        if ($request->hasFile('bulk_file')) {
            try {
                ...
                dd($e->getMessage());
                return back()->with('error', 'Failed to import file! Please check the format.');
            }
        }
    }

    public function bulk_upload_customer_price(Request $request)
    {
        if ($request->hasFile('bulk_file')) {
            $customer_detail_id = $request->input("customer_detail_id");
            $import = new ImportCustomerProducts($customer_detail_id);
            Excel::import($import, request()->file('bulk_file'));
        }
