...
class AizUploadController extends Controller
{
    ...
    public function upload(Request $request)
    {
        $type = array(
            ...
            "doc" => "document", "txt" => "document", "docx" => "document",
            "pdf" => "document", "csv" => "document", "xml" => "document",
            "ods" => "document", "xlr" => "document", "xls" => "document",
            "xlsx" => "document"
        );
        ...
        if ($request->hasFile('aiz_file')) {
            ...
            if (isset($type[$extension])) {
                ...
                for ($i = 0; $i < count($arr) - 1; $i++) {
                    $upload->file_original_name .= ($i == 0) ? $arr[$i] : "." . $arr[$i];
                }
                ...
                if ($type[$extension] == 'image' && $extension !== 'svg') {
                    try {
                        ...
                        $fullDirectoryPath = base_path('public/') . $directory;
                        $filenameOnly = pathinfo($path, PATHINFO_FILENAME);
                        
                        foreach ($sizes as $label => $widthLimit) {
                            $img = $manager->read($file);
                            
                            if ($img->width() > $widthLimit) {
                                $img->scale(width: $widthLimit);
                            }

                            
                            $newFileName = $label . '-' . $filenameOnly . '.' . $extension;
                            $img->save($fullDirectoryPath . '/' . $newFileName);
                        }

                       
                        $file->move($fullDirectoryPath, $filenameOnly . '.' . $extension);

                    } catch (\Exception $e) {
                        \Log::error("Image Resizing Error: " . $e->getMessage());
                    }
                }

                // 5. Handle Cloud Storage if needed
    ...
    //             // return $path;
    //             // exit();
    //             // $imagea = $request->file('aiz_file')->getPathname();
    //             // $aiz_files =time() . $request->file('aiz_file')->getClientOriginalName();
    //             // $pathss = base_path() . '/public/uploads/all';
    //             // $request->file('aiz_file')->move($pathss , $aiz_files);
    ...
    //                 // Define your size configurations
    //                 $sizes = [
    //                     'small'  => 300,
    //                     'medium' => 800,
    //                     'large'  => 1500
    ...
    //                 // Get basic path info (e.g., "uploads/photos/image.jpg")
    //                 $directory = dirname($path);
    //                 $filename = pathinfo($path, PATHINFO_FILENAME);
    //                 $extension = pathinfo($path, PATHINFO_EXTENSION);

    //                 foreach ($sizes as $label => $widthLimit) {
    //                     // Re-read the image for each iteration to avoid cumulative changes
    //                     $img = $manager->read($file);
                        
    //                     // Scale if the width exceeds the limit
    //                     if ($img->width() > $widthLimit) {
    //                         $img->scale(width: $widthLimit);
    //                     }

    //                     // Construct new name: e.g., "uploads/photos/small-image.jpg"
    //                     $newFileName = $label . '-' . $filename . '.' . $extension;
    //                     $savePath = base_path('public/') . $directory . '/' . $newFileName;

    //                     $img->save($savePath);
                        
    //                     \Log::info("Generated $label version: " . $newFileName);
    //                 }

    //                 // Optional: Save the original file as well
    //                 $file->move(base_path('public/') . $directory, $filename . '.' . $extension);
