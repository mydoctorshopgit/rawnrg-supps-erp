...
class ProductService
{
    ...
    public function update(array $data, Product $product)
    {
        ...
        $discount_start_date = null;
        $discount_end_date   = null;
        if ($collection->has('date_range') && $collection['date_range'] !== null) {
            $date_var = explode(" to ", $collection['date_range']);

            if (count($date_var) === 2) { // Ensure there are exactly two parts
                $discount_start_date = strtotime($date_var[0]);
                $discount_end_date = strtotime($date_var[1]);
            }
        }
        ...
        if (empty($collection['short_description'])) {
            $collection['short_description'] = substr(strip_tags($collection['description']), 0, 200);
        }
        ...
        if (isset($collection['choice_no']) && $collection['choice_no']) {
            ...
            foreach ($collection['choice_no'] as $key => $no) {
                ...
                // dd($collection);
                // foreach (json_decode($request[$str][0]) as $key => $eachValue) {
                foreach ($collection as $key => $eachValue) {
                    // dd($collection[$str]);
                    // array_push($data, $eachValue->value);
                    array_push($attribute_data, $eachValue);
                }
